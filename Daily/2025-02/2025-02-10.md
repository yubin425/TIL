...
| Namespace | 시스템 리소스를 해당 프로세스에만 해당되는 자원처럼 보이게하고, 다른 프로세스와 격리시키는 가상화 기능 | - Cgroup과 달리 접근을 제어한다<br>- PID: 프로세스 ID(PID)트리를 격리<br>- IPC: 프로세스 간 통신을 격리<br>- 네트워크, 파일 시스템 등을 분리하여 실행 가능 |
